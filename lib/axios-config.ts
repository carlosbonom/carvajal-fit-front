...
import { getAccessToken, getRefreshToken, saveTokens, clearTokens } from './auth-utils'
...
const doRefreshToken = async (): Promise<string | null> => {
  ...
  try {
    ...
    const response = await refreshAxios.post<{ accessToken: string; refreshToken: string }>('/refresh', { refreshToken: refresh })
...
setupRequestInterceptor(authAxios)
setupResponseInterceptor(authAxios)
setupRequestInterceptor(apiAxios)
setupResponseInterceptor(apiAxios)
