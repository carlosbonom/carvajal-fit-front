  ...
  try {
    ...
    if (updateError && updateError.code === "PGRST116") {
      ...
      if (insertError) {
        ...
        return NextResponse.json(
          {
            message:
              "Usuario creado exitosamente, pero hubo un error al crear el perfil",
            user: {
              id: authData.user.id,
              email: authData.user.email,
            },
            warning:
              "El perfil se creará automáticamente. Por favor, actualiza tu perfil más tarde.",
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        {
          message:
            "Usuario creado exitosamente, pero hubo un error al actualizar el perfil",
          user: {
            id: authData.user.id,
            email: authData.user.email,
          },
          warning:
            "El perfil se creó con valores por defecto. Por favor, actualiza tu perfil más tarde.",
