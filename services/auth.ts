...
export const getProfile = async (accessToken?: string): Promise<UserProfile> => {
  const response = await authAxios.get<UserProfile>('/me', accessToken ? {
    headers: {
...
export const refreshToken = async (refreshToken: string): Promise<AuthResponse> => {
  const response = await refreshAxios.post<AuthResponse>('/refresh', { refreshToken })
  return response.data
}

// POST /auth/logout - Cerrar Sesión
export const logout = async (accessToken?: string): Promise<{ message: string }> => {
  const response = await authAxios.post<{ message: string }>('/logout', {}, accessToken ? {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  } : undefined)
